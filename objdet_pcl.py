...
def intensity_to_color(intensity):
    ...
    print(intensity)
    return np.tile(np.reshape(intensity, [-1, 1]), [1, 3])
...
def show_pcl(pcl):
    ...
    #print(pcl[:,0:3].shape)
    #pcd.colors = o3d.utility.Vector3dVector(intensity_to_color(pcl[:,3:4]))
...
def show_range_image(frame, lidar_name):
    ...
    #print(img_range_intensity.shape) # (128, 2650) because 64+64

    # visualize
    # trim to +/- 90def 
    deg90 = int(img_range_intensity.shape[1]/4)
    degCenter = int(img_range_intensity.shape[1]/2)
    img_range_intensity = img_range_intensity[:, degCenter-deg90:degCenter+deg90]
...
def bev_from_pcl(lidar_pcl, configs):
    ...
    #print(lidar_pcl_cpy)
    values, indices, counts = np.unique(lidar_pcl_cpy[:, 0:2], axis=0, return_index=True, return_counts=True)
    #print(values.shape)
    #print(counts.shape)
    intensity_map_count = np.zeros((configs.bev_height + 1, configs.bev_width + 1))
    for value, count in zip(values, counts):
        intensity_map_count[int(value[0]),int(value[1])] = count
    #print(intensity_map_count)
    ...
    #plt.matshow(intensity_map_count, cmap='gray')
    #plt.axis('Equal')
    #plt.show()
   ...
   # cv2.waitKey(0)
    ...
    lidar_pcl_hei = lidar_pcl_top[idx_height]

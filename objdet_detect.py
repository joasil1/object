...
# NEEDED FOR EX3
#sys.path.append('/home/workspace/SFA3D/sfa')
#sys.path.append('/home/workspace/SFA3D/sfa/models')
#print(os.getcwd())
...
def load_configs_model(model_name='darknet', configs=None):
    ...
    if model_name == "darknet":
        ...
        # parser doesn't contain following elements
        configs.lim_x = [0, 50] # detection range in m
        configs.lim_y = [-25, 25]
        configs.lim_z = [-1, 3]
        configs.lim_r = [0, 1.0] # reflected lidar intensity
        #
        # config has following elements in different fields
        configs.bev_width = configs.input_size[0]  # pixel resolution of bev image
        configs.bev_height = configs.input_size[1] 
...
def detect_objects(input_bev_maps, model, configs):
    ...
    with torch.no_grad():  
        ...
        if 'darknet' in configs.arch:
            ...
            #print(outputs['hm_cen'].shape)
            detections_raw = decode(outputs['hm_cen'], outputs['cen_offset'], outputs['direction'], outputs['z_coor'], outputs['dim'])
            print(detedetections_rawctions)
            print(detections_raw.shape)
            detections = post_processing(detections_raw.numpy())
            print(detections)
            #######
            ####### ID_S3_EX1-5 END #######     
    ...
    if 'darknet' in configs.arch:
        objects = detections
    elif 'fpn_resnet' in cofigs.arch:
        objects = convert_det_to_real_values(detections[0])
    print(objects)
    #######
    ####### ID_S3_EX2 START #######   
